...
describe EasyWebService::RequestHandler::Http do
  let(:request) do
    OpenStruct.new(
      :app => 'foo',
  ...
  subject{ EasyWebService::RequestHandler::Http.new }
  ...
  describe "#perform" do
    ...
    context "when the action requested is registered with the handler" do
      ...
      context "when the response from the remote application is a server error" do
        ...
        it "should raise an error" do
          expect{ subject.perform(request) }.to raise_exception(EasyWebService::RequestError)
        end
      end
    end
    ...
    context "when the action is not registered with the handler" do
      it "should raise an exception" do
        ...
        expect {subject.perform(request)}.to raise_exception(EasyWebService::ActionNotFoundError)
      end
    end
  end
